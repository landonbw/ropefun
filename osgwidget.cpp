...
//    create_triangle();
    ...
    camera->setViewport( 0, -50, this->width() * pixelRatio, this->height() * pixelRatio );
    ...
    mManipulator->setAutoComputeHomePosition(true);
//    mManipulator->setHomePosition(osg::Vec3d(0,88,88),osg::Vec3d(0,0,0),osg::Vec3d(0,0,1));
...
void OSGWidget::drawSphere(Vector3 loc, double radius, double tension)
{
    ...
    osg::ShapeDrawable* sd_sphere = new osg::ShapeDrawable(sphere);
    // set the color
    double red = helperfunctions::calculateColor(tension, helperfunctions::red);
    double green = helperfunctions::calculateColor(tension, helperfunctions::green);
    double blue = helperfunctions::calculateColor(tension, helperfunctions::blue);
    //std::cout<<tension<<":\t"<<red<<", "<<green<<", "<<blue<<std::endl;

    sd_sphere->setColor(osg::Vec4(red, green, blue, 1));
    sd_sphere->setName("sphere");
    // create the geode to hold the sphere
    osg::Geode* geode = new osg::Geode;
    geode->addDrawable(sd_sphere);
...
void OSGWidget::drawOrigin()
{
    osg::Box* xBox = new osg::Box(osg::Vec3(1.0625, 0, 0), 2, 0.125, 0.125);
    osg::Box* yBox = new osg::Box(osg::Vec3(0, 1.0625, 0), 0.125, 2, 0.125);
    osg::Box* zBox = new osg::Box(osg::Vec3(0, 0, 1.0625), 0.125, 0.125, 2);
    osg::ShapeDrawable* xAx = new osg::ShapeDrawable(xBox);
    osg::ShapeDrawable* yAx = new osg::ShapeDrawable(yBox);
    osg::ShapeDrawable* zAx = new osg::ShapeDrawable(zBox);
    xAx->setColor(osg::Vec4(1, 0, 0, 0.5));
    yAx->setColor(osg::Vec4(0, 1, 0, 0.5));
    zAx->setColor(osg::Vec4(0, 0, 1, 0.5));
    osg::Geode* geode = new osg::Geode;
    geode->addDrawable(xAx);
    geode->addDrawable(yAx);
    geode->addDrawable(zAx);
...
void OSGWidget::drawRope(std::list<RopeNode *> rope, bool show_tension)
{
    for(RopeNode* node : rope)
    {
        Vector3 location = node->get_pos_vec();
        double node_tension = 0;
        if(show_tension)
        {
            node_tension = node->get_tension();
        }
        node->clear_forces();
        this->drawSphere(location, 0.5, node_tension);
